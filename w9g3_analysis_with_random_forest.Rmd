...
-change everything to week 9 names.
-combine terms and redo text analysis
...
[Model Output](#output) - same as prior week: group3_predictions_wk9.csv
...
covidclass_70percentlabels$seventy_label <- as.factor(covidclass_70percentlabels$seventy_label)
...
ili_concept <- c("\\bILI\\b", "FLU", "FLU-LIKE SYMPTOMS", "ILI - INFLUENZE LIKE ILLNESS", 	"INFLUENZA LIKE ILLNESS", "INFLUENZA", "I L I")
...
sample <- sample.split(week_9_labeled_data$id, SplitRatio = 0.75)

train  <-  subset(week_9_labeled_data, sample == TRUE)
test   <-  subset(week_9_labeled_data, sample == FALSE)
dim(train)
dim(test)
```
```{r }
...
# Old stuff for reference, delete prior to submission for week 9
...
week_9_to_vec <- setDT(week_9_labeled_data)

setkey(week_9_to_vec, id)

#Set seed and divid data with labels into test and train data
set.seed(888)
all_ids_9 <-  week_9_to_vec$id
train_ids_9 <- sample(all_ids_9, 1300)
test_ids_9 <- setdiff(all_ids_9, train_ids_9)

train_9 <-  week_9_to_vec[J(train_ids_9)]
test_9 <-  week_9_to_vec[J(test_ids_9)]

prep_fun <-  tolower
tok_fun <-  word_tokenizer

it_train_9 <- itoken(train_9$grouped_cc, 
             preprocessor = prep_fun, 
             tokenizer = tok_fun, 
             ids = train_9$id, 
             progressbar = FALSE)

vocab_9 <- create_vocabulary(it_train_9)


# Create Document Term Matrix

vectorizer_9 <- vocab_vectorizer(vocab_9)

dtm_train_9  <-  create_dtm(it_train_9, vectorizer_9)

NFOLDS <-  4

glmnet_classifier_9 <-  cv.glmnet(x = dtm_train_9, y = train_9[['seventy_label']], 
                              family = 'binomial', 
                              # L1 penalty
                              alpha = 1,
...
plot(glmnet_classifier_9)
```
```{r}
it_test_9 <-  tok_fun(prep_fun(test_9$grouped_cc))

it_test_9  <-  itoken(it_test_9, ids = test_9$id, progressbar = FALSE)
         

dtm_test_9 <-  create_dtm(it_test_9, vectorizer_9)

preds_9  <-  predict(glmnet_classifier_9, dtm_test_9, type = 'response')[,1]

predict_table_9 <-  data.frame(preds_9)
predict_9_with_id <- rowid_to_column(predict_table_9, var = "id")
predict_9_w_id_status <-  predict_9_with_id  %>%
  left_join(week_9_labeled_data, by = "id")
