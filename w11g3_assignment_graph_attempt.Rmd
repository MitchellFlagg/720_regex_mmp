...
#Join to parent id to prep for graphical analysis

covid_dataset_with_parent <- covid_dataset %>%  dplyr::left_join(relationship, by= c('ontology_id_base' = 'childID'))


summary_relationships <- relationship %>% 
  group_by(childID, parentID) %>% 
  summarise(count = n()) %>% 
  arrange(desc(count))
...
```{r, transform to tidy graph format}

graph_prep_covid <- covid_dataset %>% 
  rename(from = 'ontology_id_base') %>% 
  rename(to = 'parentID')

graph_covid <- as_tbl_graph(graph_prep_covid, directed = TRUE)
...
```{r}
ggraph(graph_covid, layout = 'kk') + 
  geom_edge_link() +
    geom_node_point()
```

```{r}
find_sink <- graph_covid %>%
  mutate(sink = node_is_sink()) 



as_data_frame(find_sink, "vertices")
```
