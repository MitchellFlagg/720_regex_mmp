...
```{r, new concept mapping}


##########
#turn chief complaint into ontology terms:

covid_dataset$cc <- as.character(covid_dataset$cc)

#join cleaned terms with qalist misspelled ontology terms
covid_dataset %<>% left_join(qalist, by=c('cc' = 'description'), ignore_case = TRUE) 

#join previous result with classic ontology terms
covid_dataset %<>%  left_join(cclist, by=c('cc' = 'description'), ignore_case = TRUE)
...
covid_dataset$ontology_id <- gsub( "-L", "", covid_dataset$ontology_id)
covid_dataset$ontology_id <- gsub( "-R", "", covid_dataset$ontology_id)
covid_dataset$ontology_id <- gsub( "-B", "", covid_dataset$ontology_id)

#remap to ontology concepts:
covid_dataset %<>% left_join(concepts, by=c('ontology_id' = 'conceptID'), ignore_case = TRUE)
...
covid_dataset %<>%  select(id, defaultDescription, ontology_id, covid_label)

colnames(covid_dataset) <- c('id', 'cc', 'ontology_id_base', 'covid_label')

#Making empty chief complaint clearer
covid_dataset$cc[which(is.na(covid_dataset$cc))] <-  "No CC provided"

covid_dataset$ontology_id_base <- as.numeric(covid_dataset$ontology_id_base)
```

```{r, examine missing terms}
ontology_bins <- covid_dataset %>% 
  dplyr::group_by(ontology_id_base, cc) %>% 
  summarise(count = n()) %>% 
  arrange(desc(count))

ontology_bins

examine_missing_ontology <- ontology_bins %>% 
  filter(is.na(ontology_id_base))

examine_missing_ontology
...
```{r, find my parent function}
find_my_parent <- function(id) {
  parent <- relationship %>% 
    dplyr::filter(childID == id) %>% 
...
find_my_parent(315596001)
```
```{r}
do_i_have_a_parent <- function(id) {
  parent_count <- relationship %>% 
    dplyr::filter(childID == id) %>% 
    nrow()
...
do_i_have_a_parent(736004
)
```
```{r}
grandest_parent <- function(id) {
  if(do_i_have_a_parent(id)) {
    return(grandest_parent(find_my_parent(id)))
  }
  else{
    return(id)
  }
...
grandest_parent(250171008)
```

```{r}
#Create top_parent from above data (takes a really long time, saved as csv and then loaded instead)
# top_parent <- covid_dataset %>% 
#   rowwise() %>% 
#   mutate(top_parent_id = grandest_parent(ontology_id_base))


#write_csv(top_parent, "./ontology/top_parent.csv")

top_parent <- read_csv("ontology/top_parent.csv")
```

```{r}
#to work with analysis pipeline need dataset to be called covid_dataset
covid_dataset <- top_parent
```
...
# Add back ontology columns and covid_label to sparse term dataframe 
tSparse$covid_label <-  labeled_data$covid_label
tSparse$ontology_id_base <-  labeled_data$ontology_id_base
tSparse$top_parent_id <-  labeled_data$top_parent_id
```
...
small_d_and_p <- data_and_prediction %>% 
  select("id", "prediction", "covid_label", "ontology_id_base", "top_parent_id")
...
full_data_top_columns$ontology_id_base <-  covid_dataset$ontology_id_base
full_data_top_columns$top_parent_id <-  covid_dataset$top_parent_id
...
### Week 10 
Linear Model
Training Data  (6539 rows with the labels you were provided)
sensitivity    	0.229426434
specificity    	0.966601499
PPV    		0.30976431
NPV    		0.950496636
accuracy    	0.921394709
f1    		0.263610315

Test/Holdout Data  (2793 rows of the remaining data whose “ground truth” labels you were NOT provided)
sensitivity    	0.2
specificity    	0.964163172
PPV    		0.265625
NPV    		0.948968105
accuracy    	0.917651271
f1    		0.228187919


Non-Linear Model (XGBoost)
Training Data  (6539 rows with the labels you were provided)
sensitivity   	0.593516209
specificity    	0.766373412
PPV    		0.142344498
NPV    		0.966509143
accuracy    	0.755773054
f1   		0.22961891

Reported Training AUC ROC = 0.680


Test/Holdout Data  (2793 rows of the remaining data whose “ground truth” labels you were NOT provided)
sensitivity    	0.570588235
specificity    	0.777735417
PPV    		0.142647059
NPV    		0.965451964
accuracy    	0.765127103
f1    		0.228235294

Holdout AUC ROC = 0.697
...
#Our COVID prediction raw probablity is in the column labeled final_predictions preds_glm_final
group3_predictions_wk11_xgboost <-  covid_dataset
write_csv(group3_predictions_wk11_xgboost, "./analysis/group3_predictions_wk11_xgboost.csv")
```
...
```{r}
#Our COVID prediction is labeled in the column final_predictions
group3_predictions_wk11_glm <- preds_glm_final 
write_csv(group3_predictions_wk11_glm, "./analysis/group3_predictions_wk11_glm.csv")
```
